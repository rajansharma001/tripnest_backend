...
const createTrip = async (req, res) => {
    try {
        const user = req.user;
        const { tripTitle, currentLocation, tripLocation, budget, startDate, endDate, tripType, numberOfPeople, } = req.body;
        if (!tripTitle ||
            !currentLocation ||
            !tripLocation ||
            !budget ||
        ...
        `;
        const secret = process.env.GEN_AI_API;
        if (!secret) {
            return res.status(404).json({ error: "Gen AI api not found." });
        }
        const genAI = new generative_ai_1.GoogleGenerativeAI(secret);
        ...
        if (!response) {
            return res.status(403).json({
                error: "Something went wrong while generating your trip. Please try again later.",
            });
        }
        let tripData;
        try {
            tripData = JSON.parse(resJson);
        }
        catch (error) {
            return res.status(400).json({ error: "Invalid JSON from AI." });
        }
        const createTrip = await tirpScehma_1.Trip.create({
